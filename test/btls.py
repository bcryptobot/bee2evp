...
def btls_server_cert(tmpdirname, server_log_file, curve, psk=False):
	...
	if psk:
		cmd = 'sudo s_server -key {} -cert {} -tls1_2 -psk 123456 -psk_hint 123 \
			-port 443 >>{} 2>>s_err.txt'.format(priv, cert, server_log_file)
	else:
		cmd = ('sudo s_server -key {} -cert {} -tls1_2 -port 443 >>{} 2>>s_err.txt'
...
def btls_client_cert(client_log_file, curve, ciphersuites, psk=False):
	for ciphersuite in ciphersuites:
		if psk:
			cmd = ('s_client -cipher {} -tls1_2 -psk 123456 -connect localhost:443 -state -nbio >>{} 2>>c_err.txt'
				.format(ciphersuite, client_log_file))
		else:
			cmd = ('s_client -cipher {} -tls1_2 -connect localhost:443 -state -nbio >>{} 2>>c_err.txt'
				.format(ciphersuite, client_log_file))

		openssl(cmd, prefix='echo test_{}={} |'.format(curve, ciphersuite))

def btls_server_nocert(server_log_file):
	cmd = ('sudo s_server -tls1_2 -psk 123456 -psk_hint 123 -nocert -port 443 >>{} 2>>s_err.txt'
...
def btls_client_nocert(client_log_file, curves_list, ciphersuites):
	for ciphersuite in ciphersuites:
		for curves in curves_list:
			if curves != 'NULL':
				cmd = ('s_client -cipher {} -tls1_2 -curves {} -psk 123456 -connect localhost:443 -state -nbio >>{} 2>>c_err.txt'
